def findPokerHand(hand):
    ...
    for card in hand:
        # We get the number and the type of card it is
        # card will be KH where K is king and H is Hearts
        if len(card) == 2:
            # For example rank
            rank = card[0]
            suit = card[1]
        else:
            # If card is 10H then length is 3 so rank will be 10 and suit will be H
            rank = card[0:2]
            suit = card[2]
        # Change the Values of A,K,Q,H so all  can be integer values
        if rank == "A":
            rank = 14
        elif rank == "K":
            rank = 13
            ...
            rank = 11
        # Once rank is achieved add it to the ranks list
        ranks.append(int(rank))
        # Append suit values to suits
        suits.append(suit)
    ...
    # set(sortedRanks) gives us only unique values in list ..then we change the values into list
